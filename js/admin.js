(() => {
  ...
  async function handleIntercomPushCallId(callId) {
    ...
    showIntercomIncomingNotification({ callId: id, community: String(d.community || cid || "").trim(), fromName, fromHouseNo: fromHouse }).catch(() => {});
  ...
  function ensureCommunitiesSubscription(user) {
    db.collection("communities").get().then((snap) => {
      ...
      try {
        startAdminIntercomIncomingListener(resolveActiveCommunityId());
      } catch {}
      if (handleHashRoute()) return;
      renderDashboard();
    }).catch(() => {
  ...
  function ensureChatphoneModal80() {
    const modal = ensureModal("chatphoneModal80", "modal-chatphone", "90%");
    if (modal.dataset.initialized === "1") return modal;
    modal.dataset.initialized = "1";
    modal.innerHTML = `
      <div class="modal-backdrop" data-modal-close="1"></div>
      <div class="modal-dialog" role="dialog" aria-modal="true" aria-labelledby="chatphoneModalTitle" style="width:min(1200px,90vw);max-width:90vw;height:90vh;display:flex;flex-direction:column;">
        <div class="modal-hd">
  ...
  function renderResidentsModule() {
    ...
    if (listEl) {
      listEl.addEventListener("click", async (e) => {
        ...
        const avatarHit = e.target && e.target.closest ? e.target.closest(".avatar-sm") : null;
        if (avatarHit) {
          try {
            await intercomStartOutgoingToResident({ communityId: cid, resident: r });
          } catch {
            toast("無法開啟通話(對講)");
          }
          return;
    ...
    const renderPendingResidents = () => {
      ...
      const renderList = (list) => {
        ...
        listEl.querySelectorAll(".resident-item").forEach(item => {
          ...
          const avatarEl = item.querySelector(".avatar-sm");
          if (avatarEl) {
            ...
            avatarEl.onclick = (ev) => {
              ...
              if (String(r.status || "").trim() === "pending") {
                toast("此帳號尚未核准，無法通話");
                return;
              }
              intercomStartOutgoingToResident({ communityId: cid, resident: r }).catch(() => toast("無法開啟通話(對講)"));
            };
          }
  ...
  async function showIntercomIncomingNotification({ callId, community, fromName, fromHouseNo }) {
    ...
    const fn = String(fromName || "住戶").trim() || "住戶";
    const fh = String(fromHouseNo || "").trim();
    const title = "生活網｜住戶來電";
    const body = [fn, fh].filter(Boolean).join("｜") || "住戶來電";
      ...
      if ("serviceWorker" in navigator) {
        ...
        if (reg && typeof reg.showNotification === "function") {
          await reg.showNotification(title, {
            ...
            data: { type: "intercom", callId: id, community: c, toRole: "admin", url },
          });
          return;
        }
      }
  ...
  function ensureIntercomIncomingModal() {
    const modal = ensureModal("intercomIncomingModal", "modal-chatcall", "90%");
    modal.style.zIndex = "1200";
    modal.style.setProperty("--modal-height", "90%");
    modal.innerHTML = `
      <div class="modal-backdrop" data-modal-close="1"></div>
      <div class="modal-dialog trae-browser-inspect-draggable" role="dialog" aria-modal="true" aria-labelledby="intercomIncomingTitle" style="width:min(1200px,90vw);max-width:90vw;height:90vh;display:flex;flex-direction:column;">
        <div class="modal-hd">
          <h3 class="modal-title" id="intercomIncomingTitle">通話</h3>
          <button class="modal-close" type="button" data-modal-close="1" aria-label="關閉">×</button>
        </div>
        <div class="modal-body chatcall-body">
          <div class="chatcall-avatar" id="intercomIncomingInitial" aria-hidden="true">
            <img id="intercomIncomingAvatar" alt="" hidden />
            <span class="intercom-avatar-text">—</span>
          </div>
          <h2 class="chatcall-name" id="intercomIncomingName">—</h2>
          <div class="chatcall-sub" id="intercomIncomingSub">—</div>
          <div class="chatcall-status" id="intercomIncomingState">等待接聽</div>
        </div>
        <div class="modal-ft" style="justify-content:center;">
          <div class="chatcall-actions">
            <button class="chatcall-btn danger" type="button" id="btnIntercomReject">掛斷</button>
            <button class="chatcall-btn success" type="button" id="btnIntercomAccept">接通</button>
          </div>
        </div>
      </div>
    `.trim();
  ...
  function ensureIntercomCallModal() {
    const modal = ensureModal("intercomCallModal80", "modal-chatcall", "90%");
    modal.style.zIndex = "1200";
    modal.style.setProperty("--modal-height", "90%");
    modal.innerHTML = `
      <div class="modal-backdrop" data-modal-close="1"></div>
      <div class="modal-dialog trae-browser-inspect-draggable" role="dialog" aria-modal="true" aria-labelledby="intercomCallTitle" style="width:min(1200px,90vw);max-width:90vw;height:90vh;display:flex;flex-direction:column;">
        <div class="modal-hd">
          <h3 class="modal-title" id="intercomCallTitle">通話</h3>
          <button class="modal-close" type="button" data-modal-close="1" aria-label="關閉">×</button>
        </div>
        <div class="modal-body chatcall-body">
          <div class="chatcall-avatar" id="intercomPeerInitial" aria-hidden="true">
            <img id="intercomPeerAvatar" alt="" hidden />
            <span class="intercom-avatar-text">—</span>
          </div>
          <h2 class="chatcall-name" id="intercomPeerName">—</h2>
  ...
  async function cleanupIntercomActive({ updateStatus, closeDelayMs = 0 } = {}) {
    ...
    try {
      if (active.localStream) active.localStream.getTracks().forEach((t) => t.stop());
    } catch {}
    try {
      const audio = document.getElementById("intercomRemoteAudioAdmin");
      if (audio) {
  ...
  async function intercomStartOutgoingToResident({ communityId, resident }) {
    ...
    const toUid = String(r.id || r.uid || "").trim();
    if (!toUid) {
      toast("找不到住戶 UID");
      return;
    }
    ...
    try {
      ...
      setIntercomCallStatus("此裝置/瀏覽器不支援通話(對講)。", true);
      return;
    }
  ...
  function startAdminIntercomIncomingListener(communityId) {
    ...
    intercomIncomingUnsub = db
      ...
      .onSnapshot(async (snap) => {
        ...
        showIntercomIncomingNotification({ callId, community: cid, fromName, fromHouseNo: fromHouse }).catch(() => {});
        setIntercomPeerVisual(prompt, {
          avatarSelector: "#intercomIncomingInitial",
          imageSelector: "#intercomIncomingAvatar",
          nameSelector: "#intercomIncomingName",
    ...
    const handleSave = async () => {
      try {
        ...
        if (state.config && typeof state.config === "object") {
          state.config.liveMeetingYoutubeUrl = youtubeUrl;
        }
        
        await configDocRef(cid).set(
          { liveMeetingYoutubeUrl: youtubeUrl, updatedAt: FieldValue.serverTimestamp() },
          { merge: true }
        );
        ...
        console.error("[LiveMeeting] 儲存失敗", e);
        toast("儲存失敗");
      }
    };
  ...
  function renderLiveMeetingModule() {
    ...
    const youtubeEmbedUrl98 = (url) => {
      const id = youtubeVideoId98(url);
      if (!id) return "";
